...
Common.prototype.wheel = function(callbackwheelup, callbackwheeldown){
	...
	document.onmousewheel   = function(ev) { fn(ev || window.event) }; 			// chrome & ie9+
	document.DOMMouseScroll = function(ev) { fn(ev || window.event) };	 		// ie8
	if (document.addEventListener) document.addEventListener('DOMMouseScroll', function(ev){ fn(ev || window.event) }, false);  // firefox
}
...
Common.prototype.lazy = function(elem){
	...
	var lazyScroll = function() {
	    var lazyArr = [], lazyElem = document.getElementsByTagName('img');
...
Common.prototype.specInitFn = function (arr) {
	let filterArr = [], classArr = [], classLen, newArr = [];
	// 分类
	arr.forEach(item => {
	...
	// 过滤
	filterArr.forEach(item => { if (item) classArr.push(item); })
	// 生成嵌套数组
	classLen = classArr.length;
	for (var i = 0; i < classLen; i++) { classArr[i].forEach(item => { item.children = classArr[i+1]; }) }
