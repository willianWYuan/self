...
// imgload2size
Common.prototype.imagesize = function(url, callback) {
    var img = new Image();
    img.src = url;
    img.onload = function() {
...
Common.prototype.getBase64Image = function(obj, callback) {
    var image = new Image();
    image.src = obj.url;
    var paramsWidth  = obj.width  || 100;
    var paramsHeight = obj.height || 100;
    ...
    image.onload = function() {
        ...
        var compressWidth, compressHeight, compressTop = 0, compressLeft = 0;
...
Common.prototype.Cookies = function() { // 需要运行在服务器
    var resultCookies = {},
        baseCode = new this.simpleTransCoding();
    resultCookies.add = function(argumentObj) { // 添加/修改
        ...
        for (var x in argumentObj) {
            ...
            var currVal = baseCode.encode(argumentObj[x].toString());
    ...
    resultCookies.get = function(params) { // 获取
        ...
        for (var i = 0; i < cookArrLen; i++) {
            var indexPosition = cookArr[i].indexOf('=');
            var k1 = cookArr[i].substring(0, indexPosition);
            var v2 = cookArr[i].substring(indexPosition + 1, cookArr[i].length);
            obj[k1] = baseCode.decode(v2);
        }
